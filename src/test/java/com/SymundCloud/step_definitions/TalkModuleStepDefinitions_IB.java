...
public class TalkModuleStepDefinitions_IB {
    ...
    public void userClicksThreeDotsButton() {
        Actions actions = new Actions(Driver.getDriver());
        actions.moveToElement(talkPageIb.threeDotsButton).click().perform();
    }

    @And("User clicks delete conversation")
    public void userClicksDeleteConversation() {
        talkPageIb.deleteConversationButton.click();
    }

    @And("User clicks yes on the pop up")
    public void userClicksYesOnThePopUp() {
        talkPageIb.yesDeleteConversationButton.click();
    }

    @Then("The conversation should not be visible")
    public void theConversationShouldNotBeVisible() {
        Assert.assertTrue(BrowserUtils.isElementVisible(talkPageIb.newlyCreatedConversation));
    }
}
